...
void Server::initConnections()
{
  ...
  if (bind(this->tcpServerFd, (struct sockaddr *)&serverAddr,
           sizeof(serverAddr)) == -1)
  ...
  if (bind(this->udpServerFd, (struct sockaddr *)&serverAddr,
           sizeof(serverAddr)) == -1)
...
void Server::sendGameData(const int fd) const
{
  ...
  const std::string serializedGameData =
      headX + headY + this->serializedGameField + "END";

  ssize_t bytesWritten =
      write(fd, serializedGameData.c_str(), serializedGameData.size());
  if (bytesWritten == -1 && (errno == EAGAIN || EWOULDBLOCK))
    std::cout << "Socket buffer is full, could not sent game data: " << fd
              << std::endl;
...
void Server::receiveDataFromClient(const int fd)
{
  ...
  if (fd == this->udpServerFd)
  {
    ...
    int n = recvfrom(this->udpServerFd, readBuf, 2, 0, (sockaddr *)&clientAddr,
                     &clientAddrLen);
    if (n == 2)
      game->updateSnakeDirection(this->addressToFd[clientAddr.sin_addr.s_addr],
                                 (int)readBuf[0]);
    else
