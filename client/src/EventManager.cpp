...
EventManager::EventManager()
    : m_hasFocus(true), m_currentState(StateType::Global) {
  LoadBindings();
}
...
bool EventManager::RemoveCallback(StateType l_state,
...
///
/// This method processes SFML events by iterating through all registered
/// bindings and their associated events. When a match is found:
/// - For keyboard events (KeyDown/KeyUp): Checks if the key code matches the
/// binding's event code
/// - For mouse button events (MButtonDown/MButtonUp): Checks if the button
/// matches and stores mouse position
...
void EventManager::HandleEvent(t_event &l_event) {
  ...
  if (eventType == EventType::MouseButtonPressed ||
    ...
    std::cout << "[EventManager::HandleEvent] Mouse event type="
              ...
              << " button=" << l_event.mouse.button << " at ("
      ...
      if (eventType == EventType::KeyPressed ||
        ...
        if (e_itr.second.m_code == l_event.keyCode) {
          std::cout << "[EventManager::HandleEvent] Match! Key code "
                    << l_event.keyCode << " matches binding '" << bind->m_name
                    << "'" << std::endl;
  ...
  for (auto &b_itr : m_bindings) {
    ...
    for (auto &e_itr : bind->m_events) {
      ...
      case (EventType::Keyboard):
        ...
        //     std::cout << "Keyboard key " << e_itr.second.m_code << " is
        //     currently pressed (continuous check)" << std::endl; if
        //     (bind->m_details.m_keyCode != -1) {
      ...
      case (EventType::Mouse):
        ...
        //     // std::cout << "Mouse button " << e_itr.second.m_code << " is
        //     currently pressed (continuous check)" << std::endl; if
        //     (bind->m_details.m_keyCode != -1) {
        //         bind->m_details.m_keyCode = e_itr.second.m_code;
        //     }
    ...
    if (static_cast<int>(bind->m_events.size()) == bind->c) {
      std::cout << "[EventManager::Update] Binding '" << bind->m_name
                << "' triggered! All " << bind->c << " events matched."
                << std::endl;
      ...
      if (stateCallbacks != m_callbacks.end()) {
        ...
        if (callItr != stateCallbacks->second.end()) {
          ...
          std::cout << "[EventManager::Update] No callback found for '"
                    << bind->m_name << "' in current state" << std::endl;
        }
      } else {
...
void EventManager::LoadBindings() {
  ...
  while (std::getline(bindings, line)) {
    ...
    if (bind && !AddBinding(bind)) {
      std::cout << "[EventManager] Failed to add binding: " << bind->m_name
                << std::endl;
      delete bind;
    } else if (bind) {
      std::cout << "[EventManager] Loaded binding: " << bind->m_name << " with "
                << bind->m_events.size() << " event(s)" << std::endl;
    }
  }
  bindings.close();
  std::cout << "[EventManager] Loaded " << m_bindings.size()
            << " bindings total" << std::endl;
}
