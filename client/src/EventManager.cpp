...
bool EventManager::AddTargetEventBindingState(std::unique_ptr<TargetEventBindingState> binding) {
...
void EventManager::HandleEvent(t_event& event) {
  ...
  for (auto& binding : bindings_) {
    ...
    for (auto& event_pair : binding_ptr->events_) {
      ...
      if (eventType == TargetEventType::KeyPressed || eventType == TargetEventType::KeyReleased) {
...
void EventManager::Update() {
  ...
  for (auto& binding : bindings_) {
    ...
    if (static_cast<int>(binding_ptr->events_.size()) == binding_ptr->matched_count_) {
      ...
      // 8. Check for global callbacks (StateType::Global - always active regardless
      // of state)
      auto other_callbacks = callbacks_.find(StateType::Global);
...
void EventManager::LoadTargetEventBindings() {
  ...
  while (std::getline(bindings, line)) {
    ...
    while (!key_stream.eof()) {
      ...
      try {
        TargetEventType type = TargetEventType(stoi(key_val.substr(start, end - start)));
